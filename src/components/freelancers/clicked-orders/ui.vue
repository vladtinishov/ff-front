...
const orderUsers = computed(() => viewer.value.ordersUsers)

const canShowFreelancersModal = ref<boolean>(false)
const selectedOrderId = ref<number>()

const isApprovedByFreelancer = (orderId: number) => {
  return orderUsers.value?.find(ou => orderId === ou.orderId)?.isApprovedByFreelancer
}
...
const onSelect = (id: number) => {
  ...
  ordersStore.setApprove({ id: selectedOrderId.value!, freelancerId: id, fromCustomer: false })

  viewerStore.getViewer()
}

const onApprove = (id: number) => {
  ordersStore.setApprove({ id, freelancerId: viewer.value.id!, fromCustomer: false })
  viewerStore.getViewer()
}
        ...
        <FSection class="w-full">
          <div :class="$style.container">
            ...
            <div :class="$style.statusBar">
              <div class="flex mt-2">
                <a-button type="primary" shape="round" @click="onApprove(item.id)" v-if="!isApprovedByFreelancer(item.id)">
                  Принять заказ
                </a-button>
